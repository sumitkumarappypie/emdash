...
export default definePlugin({
	...
	routes: {
		// ── Block Kit admin handler (dispatches page_load / block_action / form_submit) ──
		...
		admin: {
			handler: async (routeCtx: RouteCtx, ctx: PluginContext) => {
				...
				// ── page_load: render the appropriate admin page ──
				if (interaction.type === "page_load") {
					return buildAdminPage(interaction.page ?? "/", ctx);
				}

				// ── block_action: navigation or in-page actions ──
				if (interaction.type === "block_action" && interaction.action_id) {
					const id = interaction.action_id;

					// Navigation between tabs
					if (id.startsWith("nav:")) {
						const tab = id.replace("nav:", "");
						return buildAdminPage(`/${tab}`, ctx);
					}

					// Product actions
					if (id.startsWith("product:") || id === "show-create-product") {
						const result = await handleProductAction(id, interaction.value, ctx);
						if (result) return result;
						return buildAdminPage("/products", ctx);
					}

					// Category actions
					if (id.startsWith("category:") || id === "show-create-category") {
						const result = await handleCategoryAction(id, interaction.value, ctx);
						if (result) return result;
						return buildAdminPage("/categories", ctx);
					}

					// Order actions
					if (id.startsWith("order:")) {
						const result = await handleOrderAction(id, interaction.value, ctx);
						if (result) return result;
						return buildAdminPage("/orders", ctx);
					}

					// Coupon actions
					if (id.startsWith("coupon:") || id === "show-create-coupon") {
						const result = await handleCouponAction(id, interaction.value, ctx);
						if (result) return result;
						return buildAdminPage("/coupons", ctx);
					}

					// Settings actions
					if (id.startsWith("settings:")) {
						const result = await handleSettingsAction(id, interaction.value, interaction.values, ctx);
						if (result) return result;
						return buildAdminPage("/settings", ctx);
					}
				}

				// ── form_submit: handle form submissions ──
				if (interaction.type === "form_submit" && interaction.action_id) {
					const id = interaction.action_id;

					if (id.startsWith("product:") || id === "create-product") {
						const result = await handleProductAction(id, interaction.values, ctx);
						if (result) return result;
						return buildAdminPage("/products", ctx);
					}

					if (id.startsWith("category:") || id === "create-category") {
						const result = await handleCategoryAction(id, interaction.values, ctx);
						if (result) return result;
						return buildAdminPage("/categories", ctx);
					}

					if (id.startsWith("coupon:") || id === "create-coupon") {
						const result = await handleCouponAction(id, interaction.values, ctx);
						if (result) return result;
						return buildAdminPage("/coupons", ctx);
					}
				...
				// ── widget rendering ──
				if (interaction.type === "page_load" && interaction.page === "widget:revenue") {
